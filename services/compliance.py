...
FRAMEWORK_RULES: dict = {
    "GDPR": [
        ...
        },
        {
            ...
            # REPLACE this entire sql in GDPR-003
            "sql": """
                SELECT c.id, c.table_name AS name, c.full_name, c.schema_name,
                ...
                OR NOT (c.metadata ? 'lawful_basis')
                OR NOT (c.metadata ? 'processing_purpose')
        ...
        },
        {
            ...
            # GDPR-004 — change sql and sql_params
            "sql": """
                SELECT c.id, c.table_name AS name, c.full_name, c.schema_name,
                    c.database_name, c.last_seen_at, c.owner_id,
                    o.name AS owner_name,
                    EXTRACT(DAY FROM %(run_ts)s::timestamptz - c.last_seen_at) AS days_since_access
                FROM catalogs c
                LEFT JOIN owners o ON o.id = c.owner_id
                WHERE c.last_seen_at IS NULL
                OR c.last_seen_at < %(run_ts)s::timestamptz - INTERVAL '90 days'
                ORDER BY c.last_seen_at ASC NULLS FIRST LIMIT 50
            """,
            "sql_params": {"run_ts": None},   # ← None is a placeholder; will be filled at runtime
            "eval_prompt": (
                "You are a GDPR compliance auditor. "
        ...
        },
        {
            ...
            # GDPR-007 — same pattern
            "sql": """
                SELECT c.id, c.table_name AS name, c.full_name, c.schema_name,
                    c.database_name, c.updated_at, c.owner_id,
                    o.name AS owner_name,
                    EXTRACT(DAY FROM %(run_ts)s::timestamptz - c.updated_at) AS days_since_update
                FROM catalogs c
                LEFT JOIN owners o ON o.id = c.owner_id
                WHERE c.updated_at < %(run_ts)s::timestamptz - INTERVAL '180 days'
                OR c.updated_at IS NULL
                ORDER BY c.updated_at ASC NULLS LAST LIMIT 50
            """,
            "sql_params": {"run_ts": None},
            "eval_prompt": (
                "You are a GDPR compliance auditor. "
...
def make_evaluate_frameworks_node(db: DatabaseManager, llm: LLMEvaluator, rules: dict):
    ...
    def process_rule(rule, framework, run_ts_str: str):
        rule_id = rule.get("rule_id", "UNKNOWN")
        params = dict(rule.get("sql_params") or {})
        if "run_ts" in params:          # inject only for rules that need it
            params["run_ts"] = run_ts_str
        query_results = db.execute_query(rule["sql"], params)
        # None means SQL failed — treat as violation, not a pass
        if query_results is None:
            logger.error("SQL query failed for rule %s — marking as FAILED", rule_id)
            return {
                "passed": False,
                "is_error": True, 
                "issue": {
                    "issue":         f"[SQL ERROR] Could not evaluate: {rule['rule'].replace('PII/sensitive', 'PII')}",
                    "rule_id":       rule_id,
                    "framework":     framework,
                    "severity":      rule.get("severity", "high").upper(),
                    "reason":        "SQL query failed during evaluation. Check DB schema and connectivity.",
                    "dataset":       "Unknown — query failed",
                    "assignee":      "Unassigned",
                    "assigned_date": calculate_assigned_date(),
                    "due_date":      calculate_due_date(rule.get("severity", "high")),
                    "affected_rows": 0,
                    "action_url":    f"/issues/{rule_id}",
                    "status":        "OPEN - AWAITING MANUAL REMEDIATION",
                }
        ...
        # Empty list means query ran fine, no violations found
        if not query_results:
            return {"passed": True, "is_error": False, "issue": None}
        # Rows returned = violation found
        ...
        return {
            ...
            "issue": {
                ...
                "reason":        "Violation detected based on query results",
                "dataset":       extract_dataset_from_results(query_results[:5]),
                "assignee":      extract_owner_from_results(query_results[:5]),
                "assigned_date": calculate_assigned_date(),
    ...
    def evaluate_frameworks(state: ComplianceState) -> ComplianceState:
        logger.info("Node: evaluate_frameworks (FAST MODE 🚀)")

        # Guard: check if any catalog data exists at all
        catalog_count = db.execute_query("SELECT COUNT(*) AS cnt FROM catalogs")
        total_catalogs = catalog_count[0]["cnt"] if catalog_count else 0

        if total_catalogs == 0:
            logger.warning("No catalog data found — scoring as 0%, not 100%")
            for framework, fw_rules in rules.items():
                state["frameworks"][framework] = {
                    "score":        0.0,
                    "issues":       [{
                        "issue":      "No catalog data available to scan",
                        "rule_id":    "DATA-000",
                        "framework":  framework,
                        "severity":   "HIGH",
                        "reason":     "No catalogs found in the database. Compliance cannot be assessed.",
                        "dataset":    "N/A",
                        "assignee":   "Unassigned",
                        "assigned_date": calculate_assigned_date(),
                        "due_date":   calculate_due_date("high"),
                        "affected_rows": 0,
                        "action_url": "/issues/DATA-000",
                        "status":     "OPEN - AWAITING MANUAL REMEDIATION",
                    }],
                    "rules_total":  len(fw_rules),
                    "rules_passed": 0,
        ...
        # Normal flow — data exists, run all rules
        for framework, fw_rules in rules.items():
            passed_count = 0
            fw_issues = []

            with ThreadPoolExecutor(max_workers=5) as executor:
                run_ts_str = state["run_timestamp"]
                futures = [
                    executor.submit(process_rule, rule, framework, run_ts_str)
                    for rule in fw_rules
                ]
                error_count = 0
                for future in as_completed(futures):
                    ...
                    if result["passed"]:
                        ...
                        error_count += 1        # don't count errors as open issues
                        logger.warning("Rule %s skipped due to SQL error", result["issue"]["rule_id"])
                        fw_issues.append(result["issue"])
                    else:
                        fw_issues.append(result["issue"])
    ...
    return evaluate_frameworks   # ← this stays at make_evaluate_frameworks_node level
...
# ================================================================
# ENDPOINT 1: Compliance Health — reads from latest snapshot
# ================================================================
...
def get_compliance_health():
    ...
    if not snap:
        raise HTTPException(status_code=404, detail="No snapshot found. Run a scan first via /api/compliance/run.")
    ...
    return JSONResponse(content={
        "overall_compliance": snapshot_json.get("overall_compliance"),
        "compliance_health":  snapshot_json.get("compliance_health"),
        "trends":             snapshot_json.get("trends"),
    })


# ================================================================
# ENDPOINT 2: Frameworks — reads from latest snapshot
# ================================================================
...
def get_frameworks():
    ...
    return JSONResponse(content={
        "frameworks": snapshot_json.get("frameworks", [])
    })


# ================================================================
# ENDPOINT 3: Open Issues — reads from latest snapshot
# ================================================================
...
def get_open_issues():
    ...
    return JSONResponse(content={
        "open_issues": snapshot_json.get("open_issues", {})
    })


# ================================================================
# ENDPOINT 4: AI Insights — reads from latest snapshot
# ================================================================
...
def get_ai_insights():
    ...
    return JSONResponse(content={
        "ai_insights": snapshot_json.get("ai_insights", {"text": "", "beta": True})
    })

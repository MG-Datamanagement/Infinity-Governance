...
#     "personphone_id", "phone_number",
#     "customer_id", "external_person_id", "account_status", "loyalty_tier",
#     "loyalty_number", "loyalty_points", "email_consent", "sms_consent",
#     "open_id", "send_id", "subscriber_key", "email_client",
#     "from_name", "from_email", "email_id", "event_date", "list_id", "status",
#     "bounce_count", "url", "link_nameclick_id",
...
def _get_pii_tag(column_name: str) -> list:
    """Return a PII synthetic tag if column name is in PII set, otherwise no tag."""
    if column_name.lower() in PII_COLUMNS:
        return [{
...
# ============================================================================
# CATALOG OWNER MANAGEMENT
# ============================================================================

@router.patch("/api/v1/catalogs/{catalog_id}/owner", 
              response_model=CatalogInfo, 
              tags=["Catalogs"])
async def update_catalog_owner(
    catalog_id: str,
    owner_id: str = Query(..., description="New owner UUID")
):
    """
    Update the owner of a specific catalog/dataset.
    This overrides the automatic inheritance from data_source.
    """
    ...
    try:
        # 1. Verify catalog exists
        catalog = await db.fetch_one("""
            SELECT id, table_name, full_name, source_id, owner_id 
            FROM catalogs 
            WHERE id = $1
        """, catalog_id)
        ...
        # 2. Verify owner exists (recommended)
        owner = await db.fetch_one("SELECT id, name, email FROM owners WHERE id = $1", owner_id)
        if not owner:
        ...
        updated = await db.fetch_one("""
            ...
            SET owner_id = $1, 
                updated_at = NOW()
            WHERE id = $2
            RETURNING id, table_name, full_name, database_name, schema_name,
                      owner_id, updated_at
        """, owner_id, catalog_id)
        ...
        await log_api_action(
            endpoint=f"/api/v1/catalogs/{catalog_id}/owner",
            method="PATCH",
            action_summary=f"Owner updated for catalog '{catalog['full_name']}' to '{owner['name']}'",
            entity_type="catalog",
            entity_id=catalog_id,
            entity_name=catalog['full_name'],
            owner_id=owner_id,  # current user who performed the action
            request_body={"catalog_id": catalog_id, "new_owner_id": owner_id}
        )

        # 5. Return updated catalog detail
        return await get_catalog(catalog_id)  # reuse existing detailed endpoint

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error updating owner for catalog {catalog_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
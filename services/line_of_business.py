...
from models.line_of_business_schemas import DomainCreateRequest,DomainUpdateRequest
...
@router.put("/api/v1/line-of-business/{lob_id}")
async def update_line_of_business(
    ...
    existing = await db.fetch_one(
        "SELECT * FROM line_of_business WHERE id = $1",
        lob_id
    )
    ...
    updated_color = request.color or existing["color"]
    ...
    await db.execute("""
        ...
        SET name = $1,
            ...
            color = $3
        WHERE id = $4
    """, updated_name, updated_description, updated_color, lob_id)

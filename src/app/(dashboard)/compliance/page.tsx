...
function ComplianceContent() {
  ...
  const allLoading = frameworksLoading && issuesLoading && trendsLoading;
  const allError = frameworksError && issuesError && trendsError;
      ...
      <div className="grid grid-cols-1 md:grid-cols-[6fr_2fr] gap-2">
        <div className="grid grid-cols-1 gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row items-start">
                    <div className="flex-1 space-y-1">
                      ...
                      <p className="text-xs/5 text-gray-600">
                        ...
                        compliance frameworks are above 90%, with GDPR and SOC 2
                        leading at 96% and 98%. HIPAA needs attention at 79% —
          ...
          <div className="grid grid-cols-1">
            {/* Open Issues */}
            <ErrorBoundary
              fallback={<DataErrorFallback retry={refetchIssues} />}
            >
              {issuesLoading && (
                <InlineState
                  type="loading"
                  message="Loading compliance issues..."
                />
              )}
              ...
              {!issuesLoading &&
                ...
                issues.length === 0 && (
                  <InlineState
                    type="empty"
                    message="No open compliance issues."
                  />
                )}
              ...
              {!issuesLoading &&
                !issuesError &&
                issues &&
                issues.length > 0 && <ComplianceIssuesTable issues={issues} />}
            </ErrorBoundary>
          </div>
        </div>
        ...
        <div className="gap-1 border rounded-lg">
          ...
          <div className="grid grid-cols-1 gap-2 p-2 overflow-y-auto pr-1">
            {frameworksLoading && (
              <InlineState
                type="loading"
                message="Loading compliance frameworks..."
              />
            )}
            ...
            {frameworksError && (
              <InlineState
                type="error"
                message="Failed to load frameworks."
